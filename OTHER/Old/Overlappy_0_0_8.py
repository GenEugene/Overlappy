...
How to use:
	...
	- DELETE: only for "OVERLAPPY" layer delete and everything inside it. Dont forget rename and pull out layers with the animation that you need
...
v 0.0.8
	...
	- added "MOVE" button to move all overlappy keyed layers to safe layer (DELETE button cant delete them)
	- added "offsetLocator" for calculate angle offsets
	- fixed "'listObjects' is not defined"
...
####################################################################################################################
""" TO DO
	- baking only keyed attributes

	- add drag and dump attributes or conserve
	- pair rotate baking mode
...
def setText():
	...
	global savedLayerName; savedLayerName = "SAVED_Overlaps"
...
def uiCreation():
	...
	cmds.frameLayout( 'testestes', l=posScriptLabel, cll=1, bgc=frameColor1, p="main", cc=('cmds.window( "main", e=True, h=10, rtf=True )') )
	...
	cmds.frameLayout( "rotLayout", l=rotScriptLabel, cll=1, bgc=frameColor1, p="main", cc=('cmds.window( "main", e=True, h=10, rtf=True )') )
	...
	cmds.frameLayout( "checkboxLayout", l='OPTIONS', cll=1, bgc=frameColor1, p="main", cc=('cmds.window( "main", e=True, h=10, rtf=True )') )
	cmds.rowColumnLayout( numberOfColumns=3, columnWidth=[ (1, rW1), (2, rW2), (3, rW3) ] )
...
when rotation simulation bake animation with incorrect rotations.''' )
	...
	cmds.frameLayout( "buttonsLayout", l='BUTTONS PANEL', cll=1, bgc=frameColor1, p="main", cc=('cmds.window( "main", e=True, h=10, rtf=True )') )
	...
	cmds.button( label="button", command=( 'print "button 2"' ), en=False, h=buttonHeight, ann='v0.0.8+' )
	...
	cmds.button( label="button", command=( 'print "button 4"' ), en=False, h=buttonHeight, ann='v0.0.8+' )
	cmds.button( label="MOVE", command=( 'saveLayers()' ), en=True, bgc=blueButton, h=buttonHeight, ann='Move all keyed layers from "OVERLAPPY" layer to "SAVED_Overlaps"\nDELETE button cant delete saved layers' )

	cmds.separator( h=4 , style='none' )
	#||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||

	cmds.columnLayout( 'endColumn', rs=4, adj=True, p="main", bgc=endColor1 )
	cmds.frameLayout( "informLayout", l='ABOUT', cll=1, bgc=frameColor2, cl=1, p="endColumn", cc=('cmds.window( "main", e=True, h=10, rtf=True )') )
...
def translationBakingToLayer(i):
	...
	if (cycle): cmds.setInfinity( pri="cycle", poi="cycle" )
...
def saveLayers():
	...
	layerList1 = cmds.animLayer( mainLayerName, q=True, c=True )
	layerList2 = layerList1[:]
	for i in range(len(layerList1)): layerList2[i] = cmds.animLayer( layerList1[i], q=True, c=True )
		
	layerList3 = layerList2[:]
	for i in range(len(layerList2)):
		layerList3[i] = layerList2[i][0]
	
	if cmds.objExists( savedLayerName ): print savedLayerName, "layer exists"
	else: 
		print savedLayerName, "layer created"
		cmds.animLayer( savedLayerName )
	
	for i in range(len(layerList3)): cmds.animLayer( layerList3[i], e=True, p=savedLayerName )
	cmds.delete( mainLayerName )
	print mainLayerName, "layer deleted"
	...
	if ( len(listObjects) <= 1 ): print "\n||| Need to select 2 or more objects |||\n"
		...
		for i in range( len( listFixed ) ): # start rotation cycle
			...
			if ( i+1 != len( listFixed ) ):
				...
				global offsetAimLoc; offsetAimLoc = listFixed[i] + "_offsetAim_loc"
