####################################################################################################################
# Моя группа вк - https://vk.com/geneugene3d
####################################################################################################################
""" Описание

Для установки скрипта достаточно создать кнопку на шелфе и добавить весь текст скрипта в окно команды, не забыв переключить на питон

||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||

Скрипт основан на nParticle, информации о котором в интернете достаточно много.
Посмотрел несколько видео и тоже захотелось иметь такой функционал.

||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||

Комментарии к пользованию:

- Объекты, к которым применяется скрипт, должны иметь хоть какие то ключи в трансформе и ротэйте,
так как скрипт пишет анимацию на слой и при его удалении, объект без ключей в дефолтном слое не вернётся в начальное состояние.

- При включённой анимации цикла необходимо выставить границы анимации так, чтобы первый и последний кадр повторялись + ко всему анимация должна
повторяться на один цикл до и после рабочей анимационной зоны. Самый простой способ поставить PRE/POST INFINITY CYCLE, либо вручную создать повторы.

- Translation overlapper не требует особых условий, главное выделять объекты по логике их иерархии,
даже если они не связаны жесткой иерархией. После изменения нижестоящей анимации, которая влияет на более мелкие, желательно удалить слои
с мелкой анимацией и перезаписать её заново под новое движение.

- Rotation overlapper требует примерно того же, что и предыдущий подход, но выделить нужно как минимум два объекта,
так как первый является анимируемым, а второй целевым (тот на который смотрит первый). Ну и соответственно для
крайнего объекта нужно создать какой нибудь указатель в виде меша или локатора, засунуть его в этот объект и сдвинуть на расстояние,
примерно равное дистанции с родительским объектом.

-Ещё небольшая разница будет наблюдаться в создании слоёв. Мутод Translation создаст ровно столько слоёв, сколько выделено объектов,
Rotation на один меньше, так как последний выделенный объект исключается.

-Все слои с анимациями складываются в слой с именем объекта, а те в свою очередь крепятся к главному слою, который удаляется при помощи кнопки DELETE ALL.

-Кнопка RESET ALL сбрасывает все значения на ползунках translation и rotation.

-Кнопки RESET translation и RESET rotation подключу позже.

-При наведении на текст Translation description и Rotation description появится краткое описание, что делает этот блок и под какие ситуации больше подходит.

"""
####################################################################################################################
""" Фиксы
v0.02
	- Исправил ошибку, которая возникала при применении скрипта к референсному объекту (в названии есть символ ":"). Теперь при создании слоёв скрипта,
		символы "|" и ":" заменяются на "_" (нижний пробел)
		
	- Мелкие текстовые правки
"""
####################################################################################################################
""" TO DO
	- Добавить галку перезаписи существующей анимации оверлапа при запуске скрипта
...
def uiCreation():
	...
	v=goalSmooth, cc=('goalSmooth=printSmoothV()'), pre=1, cal=(1, "left"), w=slWidth, ann=" Сглаживание позиции от 0,5 до 3 ", el="~1.5" )
	
	cmds.floatSliderGrp( "s2", l='Scale', f=True, min=0.0, max=2.0, fmx=10, 
	v=timeScale, cc=('timeScale=printScaleV()'), pre=1, cal=(1, "left"), w=slWidth, ann=" Скейл скорости симуляции от 0.5 до 1 ", el="~0.3" )
	...
	v=goalSmoothRot, cc=('goalSmoothRot=printRotSmoothV()'), pre=1, cal=(1, "left"), w=slWidth, ann=" Сглаживание позиции от 0,5 до 3 ", el="~1.5" )
	
	cmds.floatSliderGrp( "s5", l='Scale', f=True, min=0.0, max=2.0, fmx=10, 
	v=timeScaleRot, cc=('timeScaleRot=printRotScaleV()'), pre=1, cal=(1, "left"), w=slWidth, ann=" Скейл скорости симуляции от 0.2 до 1 ", el="~0.3" )
...
def translationStart():
	...
	#||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
		...
		global maxLoopTime; maxLoopTime = aeT * 3
